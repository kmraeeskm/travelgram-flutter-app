...
class RatingGuide extends StatefulWidget {
  const RatingGuide({
    super.key,
  ...
  @override
  State<RatingGuide> createState() => _RatingGuideState();
}

class _RatingGuideState extends State<RatingGuide> {
...
// Read the rating value from the document
    ...
    await _firestore.collection('comments').doc(widget.guideId).set({
      'time': DateTime.now(),
    });
    await _firestore
        .collection('comments')
